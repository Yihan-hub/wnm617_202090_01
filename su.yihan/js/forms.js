...
const checkAnimalAddForm = () => {
   ...
   let health = $("#animal-add-health").val();
   let description = $("#animal-add-description").val();
   let image = $("#animal-add-image").val();

   query({
      type:'insert_animal',
      params:[sessionStorage.userId,name,type,breed,health,description,image]
   }).then(d=>{
      if(d.error) {
         throw d.error;
      }
...
const checkAnimalEditForm = () => {
   ...
   let health = $("#animal-edit-health").val();
   let description = $("#animal-edit-description").val();
   let image = $("#animal-edit-image").val();

   query({
      type:'update_animal',
      params:[name,type,breed,health,description,image,sessionStorage.animalId]
   }).then(d=>{
      if(d.error) {
         throw d.error;
      }
